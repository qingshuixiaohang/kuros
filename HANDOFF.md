...
├── user/            # 用户域：认证、资料、关注（split-02 归位；Phase B 拆出 kuros-user）
├── post/            # 帖子域（split-03）
...
### 全栈验证
- `scripts/compose-smoke.mjs`：登录 → 发帖 → 评论 → 传图 → 持久化
- `scripts/session-persistence-check.mjs`：重启后端后用旧 Cookie 验证 Redis 会话
...
7. **锁-事务竞态**：锁必须在事务外层，否则锁释放时事务未提交（切片 #5 已修）
