...
/**
 * CSRF 双重提交 Cookie 拦截器（split-06 自 kuros-backend 迁入，逐字一致）。
 * 替代 Spring Security 的 CookieCsrfTokenRepository。
 *
 ...
 *
 * 拆服务后的一致性：XSRF-TOKEN 是浏览器侧 Cookie（domain/path=/），
 * 经网关访问时前端看到的与服务端种下的一致，两侧拦截器只需同一套校验逻辑——
 * 这也是该文件与 backend 版本必须逐字相同的原因。
 */
@Component
